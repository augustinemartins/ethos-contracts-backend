...
/// Bit position of the `oracle` field within an `AttestationRecord`'s field
/// bitmask, used by [`ZkVerifierContract::redact_attestation_record`] to
/// record (via `MaskingConfig`) which fields were masked when
/// [`ZkVerifierContract::get_attestation`] serves a redacted record. Field
/// index 0 (`credential_id`) is never masked, so a redacted record still
/// identifies itself.
const ATTESTATION_RECORD_FIELD_ORACLE: u32 = 1;
/// Stellar strkey of the all-zero Ed25519 account (32 zero bytes), used by
/// [`ZkVerifierContract::masked_oracle`] as the placeholder `oracle` in a
/// redacted `AttestationRecord`. An all-zero key is not a usable Stellar
/// account, so a masked oracle can never be mistaken for (or used as) a
/// genuine attesting oracle, but it round-trips as a valid `Address`.
const MASKED_ORACLE_STRKEY: &str = "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF";
